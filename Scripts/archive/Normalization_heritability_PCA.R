...
write_tsv(df_GWA_fraction_YA, path="Processed_Data/ascr_fraction_YA.tsv", col_names = T)
...
### heritability for bpTIC norm abundance ###

df_y_bpTIC <- df_GWA_norm_abundance %>%
  dplyr::select(strain, value=norm_ascr_sum)

h2_res_bpTIC <- sommer::mmer(value~1, random=~vs(strain,Gu=A), data=df_y_bpTIC)

h2_bpTIC <- as.numeric(pin(h2_res_bpTIC, h2_bpTIC ~ (V1) / (V1+V2))[[1]][1])
h2_SE_bpTIC <- pin(h2_res_bpTIC, h2_bpTIC ~ (V1) / (V1+V2))[[2]]
h2_bpTIC
