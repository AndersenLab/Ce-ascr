...
load(file="../Ce-328pop-div/Raw/WI_328_list.RData")
load("../Ce-328pop-div/Processed_Data/indep_strain_info.RData")
...
## mecr-1 ALT distribution

ggplot()+ geom_map(data=world, map=world,
                   aes(x=long, y=lat, map_id=region),
                   color="black", fill="white", size=0.2)+
  geom_point(data = dplyr::filter(indep_strain_info, strain %in% mecr1_alt), aes(x=long, y=lat), shape =21, fill='red', size = 3, alpha = 0.9) +
  geom_point(data = indep_strain_info, aes(x=long, y=lat), fill='grey', shape =21, size = 1.2, alpha = 0.9) +
  scale_fill_brewer(palette = "Set1") +
  theme_map()+
  theme(text = element_text(size=12))
...
indep_strain_info_mecr1_alt <- indep_strain_info %>%
  dplyr::filter(strain %in% mecr1_alt)
...
geo.colours <- c("Hawaii"="burlywood3", "Australia"="#0047ab", "N. America"="#C24CF6",
                 "Europe"="#FF0009","S. America"="maroon", "Africa"="lightpink2", "Atlantic"="turquoise", 
                 "New Zealand"="darkorange1", "Asia" = "black", "Unknown" = "lightskyblue2", "Multiple"="springgreen4")
